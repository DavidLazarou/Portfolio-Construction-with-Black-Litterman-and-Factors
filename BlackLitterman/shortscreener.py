...
def get_fundamentals(ticker):
    try:
        ...
        return {
            ...
            "Beta": info.get("beta"),
            "Volatility": info.get("52WeekChange"),  # proxy for volatility
        }
    except Exception:
        return None
...
# 5. Rank & Output
# -------------------------------
shorts = df.sort_values("ShortScore", ascending=False).reset_index(drop=True)

print("\n===== Top 50 Short Candidates =====")
print(shorts[["Ticker","Sector","Industry","ShortScore"]].head(50))

# Save full ranking
shorts.to_csv("short_candidates_full.csv", index=False)
print("\nSaved full short ranking to short_candidates_full.csv")
